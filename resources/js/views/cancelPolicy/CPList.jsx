...
function CPList() {
    ...
    const [policyHide, setPolicyHide] = useState(false);
    const [cpName, setCpName] = useState('');
    const [cpDetails, setCpDetails] = useState([]);
    const [cpId, setCpId] = useState('');
    const [searchQuery, setSearchQuery] = useState('');
    const { loader, CPListData, CPCreated, CPUpdate, CPDelete } = useSelector(
        (state) => state?.cpReducer,
    );
    ...
    const handleSearch = (event) => {
        ...
        if (query.trim() === '') {
            ...
            setListingData(CPListData);
        } else {
            // Otherwise, filter the CPListData based on the search query
            const filtered = CPListData.filter((item) =>
                item.cp_name.toLowerCase().includes(query.toLowerCase()),
            );
    ...
    const handleItemClick = (index, data) => {
        ...
        setCpName(data?.cp_name);
        setCpId(data?.id);
        const newArray = data?.cp_details.split(',').map((item) => item.trim());
        setCpDetails(newArray);
        setPolicyHide(true);
        setShowPolicy('edit');
    };
    ...
    useEffect(() => {
        dispatch({
            type: actions.CP_LIST,
        });
    }, [CPCreated, CPUpdate, CPDelete]);
    return (
        <>
