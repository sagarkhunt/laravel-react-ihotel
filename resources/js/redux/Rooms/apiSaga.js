...
/**
 *
 * @param {deleteRooms} action
 */
function* deleteRooms(action) {
    const { payload } = action;
    try {
        const response = yield call(postRequest, 'delete_room', payload);

        if (response) {
            yield put({
                type: actions.ROOMS_DELETE_SUCCESS,
                payload: response.data,
            });
            if (response?.data == 'fail') {
                toast.error(response.message);
        ...
        // Dispatch the register failure action
        yield put({ type: actions.ROOMS_DELETE_FAILURE });
...
export default function* rootSaga() {
    yield all([
        ...
        takeLatest(actions.ROOMS_DELETE, deleteRooms),
